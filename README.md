...
Copy `.env.example` to `.env` in the root directory and set the values:
```env
PORT=5000
...
Start MongoDB locally before starting the API, or replace `MONGO_URI` with your
MongoDB Atlas connection string. The API fails with a clear startup error if
MongoDB cannot be reached within five seconds.
